...
class WorkplaceController(private val service:WorkplaceService){
 ...
 @GetMapping("/spaces") fun spaces(@RequestParam(required=false)floorId:Long?,@RequestParam(defaultValue="false")includeDeleted:Boolean)=ApiResponse.ok(service.listSpaces(floorId,includeDeleted))
 @GetMapping("/spaces/{id}") fun space(@PathVariable id:Long)=ApiResponse.ok(service.getSpace(id))
 @PostMapping("/spaces") @PreAuthorize(MANAGE) fun createSpace(@Valid @RequestBody r:SpaceRequest)=ApiResponse.ok(service.createSpace(r),"Space created")
 @PutMapping("/spaces/{id}") @PreAuthorize(MANAGE) fun updateSpace(@PathVariable id:Long,@Valid @RequestBody r:SpaceRequest)=ApiResponse.ok(service.updateSpace(id,r),"Space updated")
