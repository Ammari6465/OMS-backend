...
interface DeskRepository:JpaRepository<Desk,Long>,JpaSpecificationExecutor<Desk>{
 fun existsByFloor_IdAndCodeIgnoreCaseAndIsDeletedFalse(floorId:Long,code:String):Boolean;fun existsByFloor_IdAndCodeIgnoreCaseAndIdNotAndIsDeletedFalse(floorId:Long,code:String,id:Long):Boolean;fun findAllByFloor_IdAndIsDeletedFalseOrderByCode(id:Long):List<Desk>;fun existsByFloor_IdAndIsDeletedFalse(id:Long):Boolean;fun countByFloor_Building_Office_Company_IdAndIsDeletedFalse(companyId:Long):Long
 @Query("select count(d) from Desk d where d.floor.building.office.company.id=:companyId and d.isDeleted=false and (d.mode=com.sunrich.oms.workplace.DeskMode.UNAVAILABLE or d.availability=com.sunrich.oms.workplace.DeskAvailability.UNAVAILABLE)") fun countUnavailable(@Param("companyId")companyId:Long):Long
}
