...
interface OfficeRepository:JpaRepository<Office,Long>,JpaSpecificationExecutor<Office>{
 ...
 @Query("select o from Office o join fetch o.company c where c.id in :companyIds and o.isDeleted in :flags order by o.name") fun findCompanyScoped(@Param("companyIds")companyIds:Collection<Long>,@Param("flags")flags:Collection<Boolean>):List<Office>
}
...
interface BuildingRepository:JpaRepository<Building,Long>,JpaSpecificationExecutor<Building>{
 ...
 @Query("select b from Building b join fetch b.office o join fetch o.company c where c.id in :companyIds and b.isDeleted in :flags order by b.name") fun findCompanyScoped(@Param("companyIds")companyIds:Collection<Long>,@Param("flags")flags:Collection<Boolean>):List<Building>
}
...
interface FloorRepository:JpaRepository<Floor,Long>,JpaSpecificationExecutor<Floor>{
 ...
 @Query("select f from Floor f join fetch f.building b join fetch b.office o join fetch o.company c where c.id in :companyIds and f.isDeleted in :flags order by f.displayOrder, f.name") fun findCompanyScoped(@Param("companyIds")companyIds:Collection<Long>,@Param("flags")flags:Collection<Boolean>):List<Floor>
}
...
interface ZoneRepository:JpaRepository<Zone,Long>,JpaSpecificationExecutor<Zone>{
 ...
 @Query("select z from Zone z join fetch z.floor f join fetch f.building b join fetch b.office o join fetch o.company c where c.id in :companyIds and z.isDeleted in :flags order by z.name") fun findCompanyScoped(@Param("companyIds")companyIds:Collection<Long>,@Param("flags")flags:Collection<Boolean>):List<Zone>
...
interface DeskAssignmentRepository:JpaRepository<DeskAssignment,Long>,JpaSpecificationExecutor<DeskAssignment>{
 ...
 @Query("select count(a) from DeskAssignment a where a.desk.floor.building.office.company.id in :companyIds and a.isDeleted=false and a.effectiveFrom<=:date and (a.effectiveTo is null or a.effectiveTo>:date)") fun countActive(@Param("companyIds")companyIds:Collection<Long>,@Param("date")date:LocalDate):Long
...
interface SpaceAssignmentRepository:JpaRepository<SpaceAssignment,Long>,JpaSpecificationExecutor<SpaceAssignment>{
 @Query("select a from SpaceAssignment a where a.staff.id=:staffId and a.isDeleted=false and a.primaryAssignment=true and a.effectiveFrom<=:date and (a.effectiveTo is null or a.effectiveTo>:date) order by a.effectiveFrom desc") fun activeForStaff(@Param("staffId")staffId:Long,@Param("date")date:LocalDate):List<SpaceAssignment>
 @Query("select a from SpaceAssignment a where a.space.id=:spaceId and a.isDeleted=false and a.effectiveFrom<=:date and (a.effectiveTo is null or a.effectiveTo>:date) order by a.effectiveFrom desc") fun activeForSpace(@Param("spaceId")spaceId:Long,@Param("date")date:LocalDate):List<SpaceAssignment>
 @Query("select a from SpaceAssignment a where a.staff.id=:staffId and a.isDeleted=false and a.effectiveFrom<:to and (a.effectiveTo is null or a.effectiveTo>:from)") fun overlappingStaff(@Param("staffId")staffId:Long,@Param("from")from:LocalDate,@Param("to")to:LocalDate):List<SpaceAssignment>
 @Query("select a from SpaceAssignment a where a.space.id=:spaceId and a.isDeleted=false and a.effectiveFrom<:to and (a.effectiveTo is null or a.effectiveTo>:from)") fun overlappingSpace(@Param("spaceId")spaceId:Long,@Param("from")from:LocalDate,@Param("to")to:LocalDate):List<SpaceAssignment>
 @Query("select a from SpaceAssignment a join fetch a.space s join fetch s.floor f join fetch f.building b join fetch b.office left join fetch s.zone join fetch a.staff st left join fetch st.department where s.id in :spaceIds and a.isDeleted=false and a.effectiveFrom<=:date and (a.effectiveTo is null or a.effectiveTo>:date)") fun activeForSpaces(@Param("spaceIds")spaceIds:Collection<Long>,@Param("date")date:LocalDate):List<SpaceAssignment>
}
